<template>
    <section>
      ...
      <ZyModal :minWidth="650" :show="state.show.add" title="新增" key="EditChecklistInfo" @close="close">
        <EditChecklistInfo @close="close"/>
      </ZyModal>
      <ZyModal :minWidth="650" :show="state.show.edit" title="编辑" key="EditChecklistInfo" @close="close">
        <EditChecklistInfo :updateData="state.updateData" @close="close"/>
      </ZyModal>
      <ZyModal :minWidth="650" :show="state.show.view" title="查看" key="ViewChecklistInfo" @close="close">
        <ViewChecklistInfo :viewData="state.viewData" @close="close"/>
      </ZyModal>
    </section>
  ...
  <script setup>
    ...
    import EditChecklistInfo from "./edit-checklist-info.vue";

    import ViewChecklistInfo from "./view-checklist-info.vue";
    import {deleteOne, getList, updateOne} from "api/modules/api.checklist";
    import {TimeUtils} from "libs/util.time";
  
    const columns = [
      {title: '名称', dataIndex: 'name', key: 'name', align: 'center'},
      {title: '内容', dataIndex: 'items', key: 'items', align: '',customRender: ({ record }) => record.items.map(p =>p.question).join('|')},
    ...
    const statusChange = (data) => {
      updateOne({id: data.id, status: !data.status}).then(res => {
