...
const startScheduler = require('./tasks/scheduler'); // 加载并启动定时任务
...
//  db.adobeModel.sync({ alter: true })
//  db.adobeTransactionModel.sync({ alter: true }) 
 //db.userModel.sync({ alter: true })

 db.uploadsModel.sync({ alter: true })
