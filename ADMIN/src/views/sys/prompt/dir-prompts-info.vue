<template>
  ...
  <script setup>
    ...
    import GetPromptsInfo from "./get-prompts-info.vue";
    import AddPromptsInfo from "./add-prompts-info.vue";
    import ViewPromptsInfo from "./view-prompts-info.vue";
    import {promptsDelete, promptsList, promptsUpdate} from "api/modules/api.prompts";
    import {TimeUtils} from "libs/util.time";
  
    const columns = [
      {title: '名称', dataIndex: 'name', key: 'name', align: 'center'},
      {title: '内容', dataIndex: 'items', key: 'items', align: '',customRender: ({ record }) => record.items.map(p =>p.title).join('|')},
